...
def main():
    ...
    def menu_night():
        ...
        root.withdraw()
        blackwhite.pluginbw(night_path)
        root.deiconify()
    ...
    # 在菜单中添加选项
    file_menu.add_command(label="打开文件", command=menu_fileopen)
    file_menu.add_command(label="视频合成", command=menu_combine)
    file_menu.add_command(label="模拟夜视", command=menu_night)
    ...
    help_menu.add_separator()
    help_menu.add_command(label="彩蛋", command=menu_eggs)
