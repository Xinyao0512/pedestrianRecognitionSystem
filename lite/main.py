...
def main():
    ...
    # 加载背景图像
    bg_image = Image.open("./img/indexCPU.png")
    ...
    ml_advanced = Button(root, text="高级识别", width=23, height=2, highlightthickness=0, bd=0, bg="#f0f5ff", fg="gray", activebackground="#f0f5ff", font=button_font)
    ml_advanced.place(relx=0.59, rely=0.715, anchor=NW)

    ml_exitprogram = Button(root, text="帮助", width=23, height=2, highlightthickness=0, bd=0, bg="#3777f6", fg="white", activebackground="#3777f6", font=button_font)
    ml_exitprogram.place(relx=0.782, rely=0.715, anchor=NW)

    ml_server = Button(root, text="云端训练", width=23, height=2, highlightthickness=0, bd=0, bg="#f0f5ff", fg="gray", activebackground="#f0f5ff", font=button_font)
    ml_server.place(relx=0.782, rely=0.82, anchor=NW)

    eggsv = Button(root, text="ver 1.19.511_CPU", width=16, height=1 , highlightthickness=0, bd=0, bg="white", fg="black", activebackground="white", font=button_font)
    eggsv.place(relx=0.895, rely=0.965, anchor=NW)
    ...
    def combine():
        ...
        localPath = Video_Post.combiner(localPath)
        print(localPath)
        detectorlite.detector(XMLPath, localPath, TensorPath, a)
        # 显示窗口
        root.deiconify()
    ...
    def tensor():
        ...
        root.withdraw()
        if TensorPath:
            tfm.machineLearning(TensorPath, TensorPath, 1)
            root.deiconify()
        else:
    ...
    #高级识别功能
    def yolo_advanced():
        messagebox.showwarning("警告", f"请使用GPU版本")
