...
def sys_login():
    ...
    # 设置窗口背景
    bg_image = tk.PhotoImage(file='./img/login.png')
    bg_label = tk.Label(root, image=bg_image)
    bg_label.place(x=0, y=0, relwidth=1, relheight=1)

    entry_style = {"background": "#f0f5ff", "highlightthickness": 0, "border": 0}

    username_entry = tk.Entry(root, width=48, **entry_style)
    username_entry.place(relx=0.15, rely=0.56, anchor=NW)

    password_entry = tk.Entry(root, show="*", width=48, **entry_style)
    password_entry.place(relx=0.15, rely=0.68, anchor=NW)

    # 按钮
    button_font = {"size": 20, "weight": "bold"}
    login_button = tk.Button(root, text="登录", command=login,width=17, height=2, highlightthickness=0, bd=0, bg="#3777f6", fg="white", activebackground="#3777f6", font=button_font)
    login_button.place(relx=0.16, rely=0.785, anchor=NW)

    forgot_password_button = tk.Button(root, text="加密狗", command=forgot_password,width=17, height=2, highlightthickness=0, bd=0, bg="#3777f6", fg="white", activebackground="#3777f6", font=button_font)
    forgot_password_button.place(relx=0.55, rely=0.785, anchor=NW)

    #版本信息
    text_label = tk.Label(root, text="Beta 0.6.1")
    text_label.place(relx=0.85, rely=0.95, anchor=NW)
